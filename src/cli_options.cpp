...
CliOptions::CliOptions(const int argc, const char *argv[], std::string name_file_settings):m_name_file_settings(name_file_settings),m_typeserver(CliOptions::PostgreSQL){
    ...
    op_db_server.add_options()
		...
		("dbname", po::value<std::string>(), "name of database")
		("hostname,H", po::value<std::string>(), "name or address server host")
		("user,U", po::value<std::string>(), "username")
		("password,W", po::value<std::string>(), "password")
    ;
    op_all.add(op_common).add(op_db_server).add(op_backend);
    po::options_description op_visible("Allowed options");
    op_visible.add(op_common).add(op_db_server);
...
std::string CliOptions::get_all_parametrs(){
    std::stringstream ss;
	if (!m_db_name.empty())
		ss << "dbname=" << m_db_name << " ";
	if (!m_hostname.empty())
...
std::string CliOptions::get_string_typeserver(){
    switch (m_typeserver) {
